...
public class LoggingInvocationEventHandler extends AbstractInvocationEventHandler<InvocationContext> {
    private static final ImmutableList<String> MESSAGE_PATTERNS = generateMessagePatterns();
    ...
    public static LongPredicate logDurationsGreaterThan(Duration duration) {
        long durationNanos = duration.toNanos();
        return nanos -> nanos > durationNanos;
    }

    public static LoggingInvocationEventHandler of(Logger logger, LoggingLevel level) {
        return new LoggingInvocationEventHandler(logger, level, _input -> true);
    }
    ...
    private void logInvocation(Method method, @Nullable Object[] nullableArgs, long durationNanos) {
        if (isEnabled() && durationPredicate.test(durationNanos)) {
            Object[] args = (nullableArgs == null) ? NO_ARGS : nullableArgs;
    ...
    public static BooleanSupplier createEnabledSupplier(Logger logger, LoggingLevel level) {
        checkNotNull(logger, "logger");
        checkNotNull(level, "level");
        BooleanSupplier systemPropertySupplier =
                InstrumentationProperties.getSystemPropertySupplier(LoggingInvocationEventHandler.class);
        if (systemPropertySupplier.getAsBoolean()) {
            switch (level) {
                case TRACE:
    ...
    private static ImmutableList<String> generateMessagePatterns() {
